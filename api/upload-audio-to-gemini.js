...
import FormData from 'form-data'; // NEW: Import form-data library
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // FIX: Use the 'form-data' library for constructing the multipart request
        const formData = new FormData();
        formData.append('metadata', JSON.stringify(uploadMetadata), { contentType: 'application/json' });
        formData.append('file', audioBuffer, { filename: `audio_upload.${mimeType.split('/')[1]}`, contentType: mimeType }); // Pass Buffer directly
        ...
        const geminiUploadResponse = await fetch(uploadFilesApiUrl, {
            method: 'POST',
            headers: headers, // Use the headers generated by form-data
            body: formData,
        });
